...
- Credentials needed for integration:
  - Sandbox Account - you can request account creation via our BD team [sales@durian-x.com](mailto:sales@durian-x.com). After account creation we will share to you
    - Sandbox X-CLIENT-ID
    - Sandbox Secret Key
    - Sandbox Public Certificate of NAH
  - Prepare Sandbox Webhook URL
  - Prepare your own Sandbox Private Key & Public Certificate. To generate your private key and public certificate, please follow this [link](/api-reference/authentication#credential-setup). We highly recommend to differentiate sandbox and production key.

  _Please store your private key securely. The public certificate will need to be shared to NAH._
  - Request for sandbox setup to NAH. \
    ...
    - Sandbox Public Certificate to be shared to us in encrypted file to \<TBC Tech Onboarding Email\>. Password to be shared in different channel / same channel (i.e. Email, Whatsapp, Telegram) with different PIC
    - Sandbox Webhook URL
...
**3.2 Deposit Transaction Webhook Handling** [Deposit Event Webhook Details](/api-reference/transaction-notification-webhook#asset-deposited-event)
...
- Success case field:
  - status: completed
  - Header of X-Signature to be verified - [Link for Verifying Webhook Signature](/api-reference/transaction-notification-webhook#validate-webhook-signature)
...
To execute swap (on/off-ramp) you will be required to quote rate to get current rate and lock the rate. Any quotes originally created will be in active status with transition to expired or used. Quote will have expiry time to be executed marked in expiredAt. [Link for Quotes Sandbox Simulator](#quote)
...
Use your active quote to execute a swap for the transaction. referenceId can be used to refer the swap transaction from your end. The created swap will have status of processing which will move to completed/failed [Link for Swap Sandbox Simulator](#swap)
...
**Swap Webhook handling** [Swap Event Webhook Details](/api-reference/transaction-notification-webhook#swap-executed-event)
